...
- **Public booking:** `GET/POST /api/clinic/public-booking/[slug]` — unauthenticated service/slot/intake/policy discovery and online appointment request for an enabled tenant slug. Services with booking policies require explicit client acceptance before appointment creation. `GET/PATCH /api/clinic/public-booking/settings` controls the on/off switch for staff.
