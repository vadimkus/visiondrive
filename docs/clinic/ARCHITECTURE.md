...
| `ClinicProcedure` | Catalog: name, duration, hidden **`bufferAfterMinutes`** (cleanup/prep/travel), base price, currency (default AED), **booking policy** (none/deposit/full prepay/card-on-file hold), cancellation window, late-cancel/no-show fees, procedure materials/BOM, intake questions, and aftercare templates. |
| `ClinicAppointment` | Scheduled visit intent: patient, optional procedure, start/end, status, source, hidden buffer, home-visit location/travel buffers, lifecycle timestamps, **booking policy snapshot + client acceptance timestamp + payment requirement status**, **`internalNotes`**, and optional `overrideReason` for intentional scheduling exceptions. |
...
| `ClinicWaitlistEntry` | Tenant-scoped cancellation-fill queue: patient, optional service, priority, acceptable date window, preferred days/time, status, and contact/booked metadata. |
...
| `ClinicPatientPayment` | Patient-level payments (amount, patient-facing discount/fee, optional discount rule/name/reason snapshot, internal **`processorFeeCents`**, method, status, optional **`visitId`** / **`appointmentId`**). Appointment deposits reuse this ledger as `PENDING`/`PAID` payment requests with a hashed public request token and expiry; late-cancel/no-show fees reuse it as appointment-linked policy-fee payments. Refund corrections create separate `REFUNDED` adjustment rows instead of overwriting the original paid row. |
...
Public booking: `/book/[tenant.slug]` is a private branded link, not a marketplace. It is disabled by default and controlled from the clinic dashboard using `tenant_settings.thresholds.publicBooking.enabled`. The dashboard generates Google, Instagram, WhatsApp, and service-specific attributed links using UTM/source parameters. Staff can choose request-approval mode or instant-confirmation mode via `tenant_settings.thresholds.publicBooking.confirmationMode`. When enabled, the public API exposes active services, generated slots, and active service-specific intake questions. Booking creation stores DOB/contact/consent as a real patient + `ONLINE` appointment, validates required intake answers, snapshots answers into `ClinicIntakeResponse`, and still uses the scheduling guard. No public override is allowed.

Patient portal lite: `/patient-portal/[token]` is a private, token-based patient view. Tokens are stored as SHA-256 hashes, expire, and can be revoked from the patient chart. The portal exposes patient-safe operational data only: upcoming appointments, booking policy terms from appointment snapshots, aftercare/next steps, aftercare document links, package balances, receipts with payment-kind explanations, accepted consent titles, and treatment-plan progress. Requests from the portal do not mutate appointments directly; they create a `ClinicPatientPortalRequest`, CRM note, and appointment event for staff review.

Smart waitlist: `/clinic/waitlist` stores active cancellation-fill candidates in `ClinicWaitlistEntry`. Staff add a patient, optional service, priority, acceptable date window, preferred days/time, and notes. When a slot opens, `GET /api/clinic/waitlist?slotStart=...&procedureId=...` ranks candidates by service match, urgency, date fit, and wait time, then returns WhatsApp-ready copy. Contacting a patient stays manual and creates a CRM activity; booking still happens through the standard appointment flow.
...
Service areas: `/clinic/service-areas` is the solo-practitioner replacement for chain/location management. It calls `GET /api/clinic/service-areas/overview` and derives neighborhood demand from existing `ClinicPatient.area` defaults plus `ClinicAppointment.locationArea` snapshots. It reports areas served, patients missing an area, upcoming visit clusters, last-90-day completed visits, and the next visit per area without introducing branch/location tables.
...
- **Patient portal lite:** `GET/POST/PATCH /api/clinic/patients/[id]/portal-link` manages private portal links; `GET/POST /api/patient-portal/[token]` returns patient-safe portal data including appointment policy snapshots and creates reschedule/cancel/message requests; `GET /api/patient-portal/[token]/payments/[paymentId]/receipt` downloads receipt PDFs for that patient only.
- **Public booking:** `GET/POST /api/clinic/public-booking/[slug]` — unauthenticated service/slot/intake/policy discovery and online appointment request for an enabled tenant slug. `procedureId` query params preselect a service for channel/service links. Services with booking policies require explicit client acceptance before appointment creation. `GET/PATCH /api/clinic/public-booking/settings` controls the on/off switch and request-vs-instant confirmation mode for staff.
- **Booking policy foundation:** `PATCH /api/clinic/procedures/[id]` updates service-level booking policy terms. Appointment creation snapshots the selected service policy into `ClinicAppointment` so future service edits do not rewrite historical client terms.
- **Deposits/payment links:** `POST /api/clinic/appointments/[id]/actions` supports `prepare_deposit_request` (creates/reissues a pending appointment-linked deposit payment and WhatsApp-ready `/pay/deposit/[token]` request link) and `mark_deposit_paid` (manual paid mark, fee snapshot, appointment confirmation). `PATCH /api/clinic/appointments/[id]` blocks `CONFIRMED` while a required deposit remains pending.
- **Late-cancel/no-show fees:** appointment actions support `enforce_policy_fee` and `waive_policy_fee` for `LATE_CANCEL` / `NO_SHOW`. Enforced fees create pending appointment-linked payment rows (`LATE_CANCEL:*`, `NO_SHOW:*`); waived fees are event-logged and any pending fee row is voided.
