...
| `ClinicPatientPayment` | Patient-level payments (amount, patient-facing discount/fee, internal **`processorFeeCents`**, method, status, optional **`visitId`** / **`appointmentId`**). Refund corrections create separate `REFUNDED` adjustment rows instead of overwriting the original paid row. |
