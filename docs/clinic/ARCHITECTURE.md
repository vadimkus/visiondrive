...
Public booking: `/book/[tenant.slug]` is a private branded link, not a marketplace. It is disabled by default and controlled from the clinic dashboard using `tenant_settings.thresholds.publicBooking.enabled`. The dashboard generates Google, Instagram, WhatsApp, and service-specific attributed links using UTM/source parameters. When enabled, the public API exposes active services, generated slots, and active service-specific intake questions. Booking creation stores DOB/contact/consent as a real patient + `ONLINE` appointment, validates required intake answers, snapshots answers into `ClinicIntakeResponse`, and still uses the scheduling guard. No public override is allowed.
...
- **Public booking:** `GET/POST /api/clinic/public-booking/[slug]` — unauthenticated service/slot/intake/policy discovery and online appointment request for an enabled tenant slug. `procedureId` query params preselect a service for channel/service links. Services with booking policies require explicit client acceptance before appointment creation. `GET/PATCH /api/clinic/public-booking/settings` controls the on/off switch for staff.
