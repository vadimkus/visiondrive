...
- Tenant isolation is enforced server-side via `requirePortalSession()`:
  - non-master users must have an ACTIVE `tenant_memberships` record for the tenant context in their token
  - MASTER_ADMIN can switch tenant context and access any tenant

- Admin/operator mutations are now recorded in `audit_logs` (see `lib/audit.ts`) for:
  - tenant settings changes
  - user creation + membership role changes
  - zone overlay import/update
  - sensor calibration (lat/lng updates)
  - tenant creation/update/switch
  - finance expense CRUD
  - alert actions (ack/assign/resolve)
  - report subscription config
