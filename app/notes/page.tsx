...
const noteSections: NoteSection[] = [
  ...
  },
  {
    ...
    content: [
      ...
      '• ✅ Global map: all sites, status clusters, filters (tenant/city/health) — `/portal/admin/tenants`',
      '• ✅ Cross-tenant KPIs: sensors online/offline, events/min, top failing sites/sensors — `/api/portal/admin/tenants/overview`',
      '• ✅ Drilldown: tenant → site → zone → bay → sensor (reuse operator portal) — tenant switch `/api/portal/admin/tenants/switch`',
      '• ✅ /portal/admin/tenants: create/disable tenants, manage customer admins — `/api/portal/admin/tenants` + `/api/portal/admin/tenants/[id]/admins`',
      '',
      '11.6. Phase F — Analytics/Reporting (hardware + occupancy) [Week 5–6]',
      '• ✅ Sensor performance reports (rankings + trends + period comparisons) — `/portal/reports/sensors` + `/api/portal/reports/sensors`',
      '• ✅ Gateway units overview + coverage panels — `/portal/reports/gateways` + `/api/portal/reports/gateways`',
      '• ✅ Network overview graph (nodes + edges) + map pinning (MVP table; map next) — `/portal/reports/network` + `/api/portal/reports/network`',
      '• ✅ Exports + scheduled reports (weekly/monthly) — CSV: `?format=csv`; cron-ready: `/api/cron/reports` (CRON_SECRET)',
      '',
      ...
      '• ✅ Implemented portal navigation deep links on `/portal/map` (Apple Maps + Google Maps). Uses sensor coords (calibration) → bay coords → site center fallback.',
      ...
      '• ✅ Integrate app billing metrics (subscriptions, payments, churn, MRR/ARR) via Stripe — Webhook ingestion `/api/webhooks/stripe` + tables `billing_events`/`billing_subscriptions` + KPIs on `/portal/admin/finance`.',
      '• ✅ Add expenses tracking (cloud + vendors + hardware ops) → net margin + unit economics — tables `expenses` + CRUD `/api/portal/admin/finance/expenses*` + dashboard.',
      '',
